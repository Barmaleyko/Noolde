...
sys.inherits(PostgresCopyInOperation,EventEmitter);
...
function createQueryHandler(callback,blockToEnd) {
	...
	return function(err,type,data) {
		...
		switch (type) {
			...
			case Constants.MessageTypes.Backend.NoData:
				if (returned) {
					return false;
				}
...
PostgresClient.prototype.simpleQuery=function(query,block,callback) {
	if (this.closed) {
		process.nextTick(function() {
			(callback.complete || callback).call(callback,new Error("PostgresClient is no longer valid"));
		});
		return;
	}
...
PostgresClient.prototype.extendedQuery=function(query,params,block,callback) {
	...
	var b=PostgresEncoder.create()
			.pushParse("",query,[])//query,createParseTypes(query)
